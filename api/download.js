...
export default async function handler(req, res) {
	...
	const arr = req.url.split("/api/download/");
	...
	try {
		...
		if (!response.ok) {
		  return res.status(400).send("获取文件错误: " + response.status + " url: " + fileUrl);
		}
		...
		if (response.status === 302) {
		  ...
		  if (newUrl) {
			// 使用 302 跳转到新 URL
			return res.redirect(302, '/api/download?url=' + newUrl);
		  } else {
		...
		res.status(400).json({ error: 'Failed to proxy file', details: error.message });
	}
}
